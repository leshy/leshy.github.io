...
#+BEGIN_SRC python :results none :exports none :session battcalc
...
#+BEGIN_SRC python :results table :session battcalc :exports both
...
headers = ["Current (A)", "Time", "eff (%)", "cell (W)", "pack (W)"]
rows = compute(vals, [ thermal_time, efficiency, cell_power_loss, temp_rise ])
rows[0] = map(format_time, rows[0])
rows[1] = map(int, rows[1])
rows[2] = map(lambda x: f"{x:.2f}", rows[2])
...
#+RESULTS:
| Current (A) | Time  | eff (%) | cell (W) | pack (W) |
|        50.0 | 1.2h  |      97 |     0.55 |    10.30 |
|       100.0 | 19min |      95 |     2.19 |    41.19 |
|       150.0 | 8min  |      92 |     4.92 |    92.68 |
|       200.0 | 5min  |      90 |     8.75 |   164.76 |
...
wrote a cli for [[https://github.com/leshy/scad-clj-cli][clojure -> openscad compiler]] because we are not barbarians here

#+begin_src scadclj :file cube.png :results file link :exports both :axes t
(let [cell_height 70.7
      cell_r 10.625
      alpha 0.5
      batteryGreen [0.6 1 0.6 alpha]
      batterySilver [1 1 1 alpha]
      ]
  
(def cell
  (union
   (color batteryGreen (cylinder cell_r, cell_height)
   (translate [0,0,(+ (/ cell_height 2) 1) ]
              (cylinder 5 2)))))
