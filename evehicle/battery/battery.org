...
(def cellObj
  (union
   (color (:green @cellSpec) (cylinder (:r @cellSpec), (:height @cellSpec))
   (translate [0, 0, (+ (/ (:height @cellSpec) 2) 1)]
              (cylinder 5 2)))))
...
(let [
      cellD (* (:r @cellSpec) 2)
      dist (+ cellD (:dist @parallelSpec))
      centerX (- (/ (* dist (:x @parallelSpec)) 2) (/ dist 2))
      centerY (- (/ (* dist (:y @parallelSpec)) 2) (/ dist 2))
      ]
  
  (swap! parallelSpec assoc
         :dimensionX (* dist (:x @parallelSpec))
         :dimensionY (* dist (:y @parallelSpec)))

  (def parallelObj
    (translate [ (- centerX), (- centerY), 0 ]
    (map (fn [x]
           (map (fn [y]
                  (translate [ (* dist x) (* dist y) 0 ] cellObj))
                (range 0 (:y @parallelSpec))))
         (range 0 (:x @parallelSpec))))))
...
(let [
      distX (+ (:dimensionX @parallelSpec) (:dist @seriesSpec))
      distY (+ (:dimensionY @parallelSpec) (:dist @seriesSpec))
      centerX (- (/ (* distX (:x @seriesSpec)) 2) (/ distX 2))
      centerY (- (/ (* distY (:y @seriesSpec)) 2) (/ distY 2))
      ]
  
  (swap! seriesSpec assoc
         :dimensionX (* distX (:x @parallelSpec))
         :dimensionY (* distY (:y @parallelSpec)))

  (def seriesObj
    (translate [ (- centerX), (- centerY), 0 ]
               (map (fn [x]
                      (map (fn [y]
                             (translate [ (* distX x) (* distY y) 0 ] parallelObj))
                           (range 0 (:y @seriesSpec))))
                    (range 0 (:x @seriesSpec))))))
