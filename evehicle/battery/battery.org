...
(defn printDimsHeader []
  (println "| Name | dimX (mm) | dimY (mm) | dimZ (mm) | Vol (cm³) |")
  (println "|-"))

(defn printDim [name obj]
  (let [
        volume (Math/round (/ (reduce * (:dims obj)) 1000))
...
; (repeatObj [1 0 0] 20 2
 (rotateObj [1 0 0]
            (repeatObj [1 0 0] 20 4
 (rotateObj [0 0 0] (repeatObj [0 0 1] 5 5 rotCellObj)))));)
...
(printDims "pack1" pack1 "pack2" pack2 "pack3" pack3 "pack4" pack4)
#+end_src

#+RESULTS:
:results:
| Name  | dimX (mm) | dimY (mm) | dimZ (mm) | Vol (cm?) |
|-------+-----------+-----------+-----------+-----------|
| pack1 |     317.5 |     460.0 |      70.7 |     10326 |
| pack2 |     373.5 |     460.0 |      47.5 |      8161 |
| pack3 |     250.0 |     373.5 |     100.0 |      9338 |
| pack4 |     220.0 |     317.5 |     161.4 |     11274 |

:end:

  _actually pack2/3 seem very interesting, why is this an uncommon HP or DIY format?_
...
(def pack20SObj (pairObj [1 0 0] (:space @packSpec) pack10SObj pack10SObj))
...
- needs plastic isolation on sides of bus bar to prevent shorts _concerns about retention, vibration etc but solvable_
- probably battery is in a sandwich of plexyglass

likely plastic insulation can double as a cell retention frame?
...
- bus bars need to exist but their function is only balancing and ideally have minimal current passing through? fusing is still possible?
...
_Current conclusion is that some sort of common nickel strips will work well as fuses as well given these are high power cells and will burn the strip in case of a cell level short_
