...
openscad is not sustainable, wrote a cli for [[https://github.com/leshy/scad-clj-cli][clojure -> openscad compiler]] so starting from scratch
...
_Note that here bus bar is not in contact with actual cells, likely to eliminate vibration of the plate damaging battery insulation_


* Starting with scad-clj
we define a cell

##+view: ./3d/cell1.png 0,0,0,0,0,90,300
##+view: ./3d/cell2.png 0,0,0,90,0,0,300
#+begin_src scadclj :exports both :axes t :session batt
...
(defn make-row [x xDist objFn yn ySpacing]
  (reduce (fn [inner-acc y]
            (let [obj (objFn x y)
                  yDist (+ (:dimY obj) ySpacing)]
              {:objs (conj (:objs inner-acc) 
                          (translate [(* xDist x) (* yDist y) 0] (:obj obj)))
               :dimY (+ yDist (:dimY inner-acc))}))
          {:objs [] :dimY 0}
          (range 0 yn)))

(defn make-grid [xn yn xSpacing ySpacing objFn]
  (reduce (fn [acc x]
            (let [obj (objFn x 0)
                  xDist (+ (:dimX obj) xSpacing)
                  row-result (make-row x xDist objFn yn ySpacing)]
              {:objs (conj (:objs acc) (:objs row-result))
               :dimX (+ xDist (:dimX acc))
               :dimY (:dimY row-result)}))
          {:objs [] :dimX 0 :dimY 0}
          (range 0 xn)))

(defn spreadObjectF [xn yn xSpacing ySpacing objFn]
  (let [result (make-grid xn yn xSpacing ySpacing objFn)]
    (->Obj (:dimX result) (:dimY result) (:objs result))))



; function to stack some object on X and Y dimensions, used later
(defn spreadObject [xn yn xSpacing ySpacing obj]
  (let [
        xDist (+ (:dimX obj) xSpacing)
        yDist (+ (:dimY obj) ySpacing)
        centerX (- (/ (* xDist xn) 2) (/ xDist 2))
        centerY (- (/ (* yDist yn) 2) (/ yDist 2))
        ]

  (->Obj
   (* (+ xSpacing (:dimX obj)) xn)
   (* (+ ySpacing (:dimY obj)) yn)
...
** a parallel block
#+begin_src scadclj :file ./3d/parallel_block.png :results file link :exports both :axes t :session batt :codefile code.clj
(let [ dist 5 x 4 y 2 ]
  ; need to repeat objects while matching cell positions,
  ; will be used for bus bar for example
  (defn parallelObjf [cellObj] (spreadObject x y dist dist cellObj)))

(def parallelObj (parallelObjf cellObj))
(:obj parallelObj)
#+end_src

#+RESULTS:
[[file:./3d/parallel_block.png]]


** a full pack
#+begin_src scadclj :file ./3d/series_block.png :results file link :exports both :axes t :session batt :codefile code.clj
(let [ dist 10 x 4 y 5 ]
  (def seriesObj (spreadObject x y dist dist parallelObj)))

(:obj seriesObj)
#+end_src

#+RESULTS:
[[file:./3d/series_block.png]]


* Bus bar
** initial design
##+view: ./3d/busbar2.png
#+view: ./3d/busbar3.png 0,0,0,0,0,90,1300
#+view: ./3d/busbar1.png 0,0,0,90,0,0,1300
#+begin_src scadclj :axes t :session batt :exports both :scadfile battpair.scad :codefile code.clj
(defn rescaleColor [r g b a] [(/ r 255) (/ g 255) (/ b 255) a])
...
(let [
      ...
      busBarDist 4
      dist 10
      moveDist (/ (+ dist (:dimY parallelObj)) 2)
      dimX (:dimX parallelObj)
      dimY (+ (* (:dimY parallelObj) 2) dist)
      ]

  (defn createPair_old [obj] (->Obj dimX dimY
  (union 
   (translate [0 moveDist 0] (:obj obj))
   (rotate [0 Math/PI 0]
          (translate [0 (* moveDist -1) 0] (:obj obj))))))


  
(defn createPair [obj]
  (spreadObjectF 1 2 dist dist
    (fn [x y]
      (if (= (mod y 2) 0)
        obj
        (->Obj (:dimX obj) 
               (:dimY obj)
               (rotate [0 Math/PI 0] (:obj obj)))))))
  ...
  (def singleHole
    (->Obj
     (:dimX cellObj)
     (:dimY cellObj)
     (cylinder (+ (/ (:dimX cellObj) 2) busBarSpace), (+ busBarThicc 2))))

  (def busBarCutter (createPair (parallelObjf singleHole)))

  (def busBar (translate
               [0 moveDist (+ (/ (:height @cellSpec) 2) (+ (/ busBarThicc 2) busBarDist))]
               (union
                (cube (:dimX busBarCutter) (:dimY busBarCutter) busBarThicc)
                (:obj busBarCutter))))

  ;; (binding [*out* *err*]
  ;; (println "eleement dimX" (:dimX parallelObj) "dimY" (:dimY parallelObj))
  ;; (println "cutter dimX" (:dimX busBarCutter) "dimY" (:dimY busBarCutter)))
  
  (def busBarCombo
    (->Obj
     (:dimX parallelObj)
     (:dimY parallelObj)
     (union (color (rescaleColor 255 186 122 1) busBar)
            (:obj parallelObj))))

  (def twoSeries (createPair busBarCombo))
)

(:obj (spreadObjectF 3 3 10 10 (fn [x y] parallelObj)))
...
soldering video: https://www.youtube.com/watch?v=cWQWDyBtgTs
