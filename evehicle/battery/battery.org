...
(def nickelStripsObj
  (let [
      ...
      nickelLength xDim
      nickelThicc 1
      nickelColor [0.8 0.8 0.8]]
      (repeatObj [0 1 0] (:space @cellGroupSpec) 2
      (->Obj [nickelLength (:d @cellSpec) nickelThicc]
             (color nickelColor 
                    (cube nickelLength nickelWidth nickelThicc)
                    )))))

(def busBarNickelObj (->Obj (:dims busBarObj) (union
    (:obj busBarObj)
    (translate [0 0 (/ thicc 1.9) ] (:obj nickelStripsObj)))))

(def busBarPairY
   (let [
     midSpace (- (:space @packSpec) padding)
     fakeYDim (nth (:dims busBarNickelObj) 1)
    ] (->Obj (:dims busBarNickelObj)
        (translate [0 (/ (+ midSpace fakeYDim) 2) 0]
        (union
         (:obj (pairObj [0 1 0] midSpace busBarNickelObj busBarNickelObj))
         (color copperColor (cube xDim midSpace thicc))
         )))))


)

(:obj (seqObj [0 0 1] 0.1 [ (flipObj busBarNickelObj) parallelCellsObj busBarNickelObj ]))
#+end_src
...
#+view: ./3d/initialPacks.png
#+begin_src scadclj :exports both :axes t :session batt :codefile code.clj :scadfile code.scad
(def pack1SObj
   (->Obj (:dims parallelCellsObj) (:obj (pairObj [0 0 1] 0
           ; this translate is just for openscad to color correctly
           (translateObj [0 0 0.1] parallelCellsObj) 
           busBarPairY))))

(def pack1SPreview (pairObj [0 1 0] (:space @packSpec)
   pack1SObj
   (rotateObj [0 0 2] parallelCellsObj)
))

(def pack2SObj (pairObj [0 1 0] (:space @packSpec)
   pack1SObj
   (rotateObj [0 0 2] (flipObj pack1SObj))
))

(def pack4SObj (pairObj [0 1 0] (:space @packSpec) pack2SObj pack2SObj))
(def pack5SObj (pairObj [0 1 0] (:space @packSpec) pack4SObj parallelCellsObj))
