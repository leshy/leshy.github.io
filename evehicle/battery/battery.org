...
; function to repeat some object on X and Y dimensions, used later
(defn spread [xn yn xDist yDist xSize ySize obj]

  (let [
        distX (+ xSize xDist)
        distY (+ ySize yDist)
        centerX (- (/ (* distX xn) 2) (/ distX 2))
        centerY (- (/ (* distY yn) 2) (/ distY 2))
        ]
    ...
    (translate
     ...
     (map (fn [x]
            (map (fn [y]
                   (translate [ (* distX x) (* distY y) 0 ] obj))
                 (range 0 yn)))
          (range 0 xn)))))

(defn spreadObject [xn yn xDist yDist obj]
  (->Obj
   (* (+ xDist (:dimX obj)) xn)
   (* (+ yDist (:dimY obj)) yn)
   (spread xn yn xDist yDist (:dimX obj) (:dimY obj) (:obj obj))))


(defrecord Obj [dimX dimY obj])

(let [r 10.625 d (* r 2) height 70.7 green [0.6 1 0.6 0.5]]
  (def cellObj (->Obj d d
                      (union
                       (color green (cylinder r, height)
...
(let [ dist 5 x 4 y 2 ]
  (def parallelObj (spreadObject x y dist dist cellObj)))
(:obj parallelObj)
#+end_src
